...
def parse_args():
    ...
    # Never use this script with args.gradient_accumulation_steps == 1
    if args.gradient_accumulation_steps == 1:
        raise ValueError("gradient_accumulation_steps should be greater than 1")
    return args
...
def main():
    ...
    # # meaningful tokens are those tokens whose labels are not IGNORE_INDEX
    # num_meaningful_tokens = 0
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        with TorchTracemalloc() as tracemalloc:
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                shift_token_weights = shift_token_weights.to(shift_logits.device)
                ...
                current_loss=-torch.sum(shift_token_weights*log_probs_for_loss)
                
                # accelerator.backward does division by args.gradient_accumulation_steps, so adjust if before itself
