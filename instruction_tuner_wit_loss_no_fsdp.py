...
class DataCollatorForInstructionTuning:
    ...
    def __call__(self, features: List[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, attention_mask, labels, token_weights = tuple([torch.tensor(feature[key]) for feature in features] for key in ["input_ids", "attention_mask", "labels", "token_weights"])
        ...
        token_weights=rnn.pad_sequence(token_weights, batch_first=True, padding_value=IGNORE_INDEX)
        return dict(
            input_ids=input_ids,
...
def parse_args():
    ...
    parser.add_argument("--dataset_name_or_path", help="Dataset name(in 🤗 datasets hub) or path to a local dataset")
    ...
    parser.add_argument("--prompt_tokens_weight", type=float, default=0.0, help="The weight for prompt tokens to be used in the loss")
    parser.add_argument("--response_tokens_weight", type=float, default=1.0, help="The weight for response tokens to be used in the loss")
...
def main():
    ...
    def preprocess_function(examples):
        ...
        all_prompts=[p[:-1] if p.endswith(" ") else p for p in examples["prompt"]]
        all_responses=[r.replace("###eos_token###", tokenizer.eos_token) for r in examples["response"]]

        prompts_responses=[p+" "+r for p, r in zip(all_prompts, all_responses)]
        prompts_responses_tokenized=tokenizer(prompts_responses, truncation=True, max_length=max_seq_length)
        prompts_tokenized=tokenizer(all_prompts, truncation=True, max_length=max_seq_length)
        ...
        if args.prompt_tokens_weight==0.0:
            for labels, prompt_len in zip(all_labels, prompts_len):
                labels[:prompt_len]=[IGNORE_INDEX]*prompt_len
        if args.response_tokens_weight==0.0:
            for labels, prompt_len in zip(all_labels, prompts_len):
                labels[prompt_len:]=[IGNORE_INDEX]*len(labels[prompt_len:])
        all_token_weights=[]
        for labels, prompt_len in zip(all_labels, prompts_len):
            all_token_weights.append([args.prompt_tokens_weight]*prompt_len+[args.response_tokens_weight]*(len(labels)-prompt_len))
        result={k: v for k, v in prompts_responses_tokenized.items()}
        result["labels"]=all_labels
        result["token_weights"]=all_token_weights
        return result
    ...
    remainder = num_samples_in_epoch % args.gradient_accumulation_steps
    remainder = remainder if remainder != 0 else args.gradient_accumulation_steps
    total_gradient_updates = math.ceil(num_samples_in_epoch / args.gradient_accumulation_steps)

    # We need to recalculate our total training steps as the size of the training dataloader may have changed.
    # num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    num_update_steps_per_epoch=total_gradient_updates
    ...
    for epoch in range(args.num_train_epochs):
        with TorchTracemalloc() as tracemalloc:
            ...
            for update_step in range(total_gradient_updates):
                # In order to correctly calculate the total number of non-padded tokens on which we'll compute the cross-entropy loss
                # we need to pre-load the full local batch - i.e the next per_device_batch_size * accumulation_steps samples
                batch_samples = []
                num_batches_in_step = (
                    args.gradient_accumulation_steps if update_step != (total_gradient_updates - 1) else remainder
                )
                for _ in range(num_batches_in_step):
                    batch_samples += [next(training_iterator)]
                # get local num items in batch
                local_num_items_in_batch = sum([(batch["labels"][...,1:].ne(IGNORE_INDEX)).sum() for batch in batch_samples])
                ...
                for i, batch in enumerate(batch_samples):
                    ...
                    with ctx():
                        ...
                        token_weights=batch.pop("token_weights")
                        ...
                        shift_token_weights = shift_token_weights.view(-1)
                        ...
                        shift_token_weights=shift_token_weights[ignore_index_mask]
                        ...
                        loss=-torch.sum(shift_token_weights*log_probs_for_loss)
                        ...
                        loss = loss * args.gradient_accumulation_steps * accelerator.num_processes

                        del labels, token_weights, outputs, ignore_index_mask, logits, shift_logits, shift_labels, shift_token_weights, log_probs, log_probs_for_loss
                ...
                if args.with_tracking:
                    accelerator.log({"instant_loss": losses, "lr": optimizer.param_groups[0]["lr"], "step":completed_steps}, step=completed_steps)
        ...
        with TorchTracemalloc() as tracemalloc:
            for step, batch in enumerate(eval_dataloader):
                with torch.no_grad():
                    ...
                    # Get the log_probs only corresponding to the labels
                    log_probs_for_loss=log_probs[range(len(shift_labels)), shift_labels]
                    loss=-torch.mean(shift_token_weights*log_probs_for_loss)
